...
如果你想确保数据仍然在那里，就必须真正地读取并检查。大多数情况下它仍然在那，但如果不是你一定希望尽早发现。由于同样的原因，尝试时不时地从备份中恢复是很重要的——否则，当数据已经丢失的时候才发现备份也虽坏，就太晚了。不要盲目地相信所有一切都正常工作。
...
如果我们不能完全相信每一个系统组件都不会被破坏——也就是每一块硬件都不会出故障，每一款软件都没有bug——那么我们至少必须定期检查数据的完整性。如果我们不检查，我们就不会发现破坏直至为时已晚，造成下游的一些损害，在那里追踪问题会更难、代价会更高。

检查数据系统的完整性最好采用端到端的方式（见“数据库端到端的参数”一节）：在完整性检查中我们可以包含的系统越多，数据破坏在流程的某个阶段没有被注意的可能性就越低。如果我们可以检查整个派生数据管道从端到端是否正确，那么路径上的任何磁盘、网络、服务和算法都隐式地包含在检查中。

连续的端到端完整性检查可以让你对系统的正确性有了更大的信心，这反过来又使你的行动更加迅速。与自动化测试一样，审计增加了可以快速发现bug的几率，从而降低了系统变更或是新存储技术造成损害的风险。如果你不担心进行更改，就可以更好地改进应用以满足不断变化的需求。

#### 为可审计数据系统准备的工具

当下，还没有很多数据系统把可审计性作为高度关注的问题。一些应用实现了自己的审计机制，比如把所有变更记录到一张单独的审计表中，但是保证审计日志和数据库状态的完整性仍然很难。事务日志可以通过定期使用硬件安全模块对其进行签名从而防止篡改，但这没法保证正确的事务首先进入日志。

使用加密工具来证明一个系统可以健壮到轻松应对软硬件问题、甚至是恶意攻击这样的完整性是很有趣的。加密货币、区块链，以及分布式账本，比如比特币、以太坊、Ripple、Stellar以及其他各种技术都涌现出来探索这一领域。

我没有资格评论这些技术作为货币或协议合同机制的优点。然而从数据系统的角度来看，它们包含了一些有趣的想法。本质上，它们是分布式数据库，有数据模型和事务机制，不同的副本可以由相互不信任的组织托管。副本不断检查彼此的完整性，并使用协商一致的协议来商定应该执行的事务。

我对这些技术在拜占庭式容错方面有点怀疑（见“拜占庭式故障”一节），我发现*工作证明*（比如，比特币挖矿）的技术非常浪费。比特币的交易吞吐量相当低，尽管更多的是出于政治和经济原因，而不是技术原因。然而，完整性检查这个方面还是有趣的。

密码审计和完整性检查通常依赖*墨克树*[74]，它们是哈希树，可以用来有效地证明记录出现在某些数据集中（以及其他一些东西）。除了加密货币的炒作之外，*证书透明度*是一种基于墨克树的安全技术，用来检查TLS/SSL证书的有效性。

我可以想象完整性检查与审计算法，比如证书透明度和分布式账本，在一般的数据系统中得到了更广泛的应用。需要做一些工作使它们与无需加密审核的系统具有同等的可伸缩性，并尽可能地降低性能损失。但我认为这是未来值得关注的一个有趣领域。

## 做正确的事

在这本书的最后一节，我想退一步。在整本书中，我们研究了各种不同的数据系统架构，评估了它们的优缺点，并探索了构建可靠的、可扩展的以及可维护的应用的技术。然而，我们漏掉了关于讨论的一个重要的基本部分，现在我想补充一下。

每个系统的构建都是有目的的；我们所采取的每一项行动都有处于意料之中也有意料以外的后果。目的可能和赚钱一样简单，但对世界的影响可能远远超出最初的目的。我们，作为建造这些系统的工程师，有责任仔细考虑这些后果，并有意识地决定我们想要生活在什么样的世界。

我们把数据说成是抽象的东西，但是记住，很多数据集都是关于人的：他们的行为，他们的兴趣，他们的身份。我们必须以人道以及尊重的态度对待这些数据。用户也是人，人的尊严是至高无上的。

软件开发越来越多地涉及到做出重要的道德选择。现在有帮助软件工程师解决这些问题的指导方针，比如ACM的“软件工程伦理和专业实践守则”，但是实践中人们很少讨论、应用或是执行它。因此，工程师和产品经理有时对其产品的隐私以及潜在的负面后果采取非常轻率的态度。

技术本身没有好坏之分——重要的是它是如何使用的以及它是如何影响人们的。对于像搜索引擎这样的软件系统来说，这与枪一样的武器是类似的。我认为对于软件工程师来说，仅仅关注技术而忽视其后果是不够的：我们也应该承担伦理责任。关于伦理的推理很困难，但是它太重要了，无法忽视。

### 预测分析

例如，预测分析是“大数据”炒作的一个主要部分。使用数据分析来预测天气或疾病的传播是一回事；预测罪犯是否有可能再次犯罪，贷款申请人是否有可能违约，或着保险客户是否可能提出昂贵的索赔要求则是另外一回事。后者直接影响到个人的生活。

当然，支付网络希望防止欺诈性交易，银行希望避免不良贷款，航空公司希望避免劫持，公司希望避免雇佣低效率或是不值得信赖的人。从他们的观点来看，失去一次商业机会的成本很低，但是一笔不良贷款或是一位有问题的员工的成本要高得多，所以组织很自然地要谨慎行事。如果有疑问，他们最好拒绝它。

然而，随着基于算法的决策越来越普遍，被某些算法（准确地或错误地）标记为有风险的人可能会承受大量“不”决策。系统性地被排除在工作、航空旅行、保险范围、财产租赁、金融服务以及其他关键的社会方面之外，这是对个人自由的巨大限制，因此被称为“算法监狱”。在尊重人权的国家，刑事司法系统在证明有罪之前会假定无罪；另一方面，自动化系统在没有任何犯罪证据的情况下可以系统地、任意地排除一个人参与社会活动，并且几乎没有上诉的机会。

#### 偏见与歧视

算法所做的决定不一定比人类的决策更好，或是更糟糕。每个人都会有偏见，哪怕他们积极地尝试消除偏见，歧视性的做法也可能在文化上制度化。人们希望基于数据作出决定，而不是人们主观本能的判断，这样会更加公平，并给在传统系统中经常被忽视的人提供更好的机会。

当我们开发预测分析系统时，我们不仅仅是通过软件来指定什么时候说“是”或“不”的规则来实现人类决策的自动化；我们甚至从数据中推断规则。然而，这些系统学习到的模式是不透明的：即使数据中存在某种关联性，我们也可能不知道原因。如果算法的输入存在系统性偏差，系统很可能会学习它，并且在输出中放大它。

在许多国家，反歧视法禁止根据例如种族、年龄、性别、性、残疾或信仰等受保护的特征对人进行不同的对待。一个人的数据的其他特征可能会被分析，但是如果它们与受保护的特征相关会发生什么呢？例如，在种族隔离的社区，一个人的邮政编码，甚至他们的IP地址，都是种族的有力预测因素。这样说，认为一个算法可以接受有偏见的数据作为输入，从中产生公平和公正的输出，这似乎是荒谬的。然而，这种信念似乎常常被数据驱动决策的支持者所暗示，这种态度被讽刺为“机器学习就像为偏见洗白”。

预测分析系统仅仅是从过去推断出来的；如果过去是歧视性的，它们就会把这种歧视代码化。如果我们想让未来比过去更美好，就需要有道德想象力，而这只有人类才能提供。数据和模型应该是我们的工具，而不是我们的主人。

#### 责任和问责

自动化地做决策引发了责任和问责的问题。如果一个人犯了错误，他们可以被追究责任，受决策影响的人可以上诉。算法也会犯错，但是如果算法出错谁来负责呢？当自动驾驶汽车发生事故的时侯谁来负责？如果自动信用评分算法系统性地歧视某一特定种族或拥有特定宗教信仰的人们，有补救办法吗？如果你的机器学习系统的决定受到司法审查，你能向法官解释算法是如何做出决定的吗？

信用评级机构是一个收集数据以做出有关人的决策的老例子。很低的信用评分会让生活变得困难，但是至少信用评分一般是基于人的实际借贷历史的相关事实，记录中的任何错误都可以纠正（尽管让评级机构做到这一点通常很不容易）。然而，基于机器学习的评分算法通常使用范围更广、更不透明的输入，使人们更难以理解某项决定是如何做出的，也很难判断人们是否受到了不公平或是歧视性的待遇。

信用评分可以总结“你过去的表现怎么样？”，而预测分析通常是建立在“你与谁很相似，过去像你这样的人是如何行动的？”的基础上的。把对方与其他人的行为相提并论意味着对人的成见，比如说基于他们居住的地方（它意味着种族与社会经济阶层）。那些被错误归类的人怎么办呢？此外，如果一项决定因为数据错误而不正确，追索几乎是不可能的。

许多数据天然拥有统计性质，这意味着即使总体上概率分布是正确的，个别情况也很可能是错误的。例如，如果你的国家的平均预期寿命是80岁，那并不意味着你会在你80岁生日那天死去。从平均值与概率分布来看，你无法说出一个人会活多久。同样，预测系统的输出是概率性的，它很可能在个别情况下是错误的。

盲目相信数据在决策中拥有至高无上的地位，这不仅是妄想，而且是非常危险的。随着数据驱动的决策变得越来越广泛，我们需要弄清楚如何使算法变得透明、变得可问责，如何避免强化现有的偏见，以及如何在它们不可避免地出错时修复它们。

我们还需要搞清楚如何防止数据被用来伤害人们，而是发挥其积极方面的潜力。比如说，分析可以揭示人们生活的财务与社会特征。一方面，这种力量可以用来集中援助与支持以帮助那些最需要援助的人。另一方面，它有时会被掠夺性企业利用，试图识别出弱势群体，从而向他们出售高成本贷款和毫无价值的大学学位等高风险产品。

#### 环形反馈

即使是那些对人的影响较小的预测应用，比如说推荐系统，我们也必须面对一些棘手的问题。当服务已经擅长预测用户想要看到的内容时，他们最终可能只向人们展示他们已经同意的观点，从而形成了回音室，刻板印象、错误信息以及两极分化在这里滋生。我们已经看到社交媒体回音室对于选举的影响。

预测分析影响了人们的生活，特别是由于自我强化的反馈回路而产生的有害问题。举个例子，考虑一下雇主使用信用评分来评估潜在雇员的场景。你可能是一名优秀的员工，信用分数很高，但突然发现自己陷入了你无法控制的不幸导致的经济困境。当你错过账单上的还款时间，你的信用分数会降低，你找到工作的可能性也降低了。失业使你陷入贫困，这进一步恶化了你的分数，使你更难找到工作。这是一个恶性假设导致的向下螺旋，隐藏在严谨的数学、数据的伪装之下。

我们没有办法总能预测这样的反馈循环什么时候发生。然而，许多后果可以通过考虑整个系统（不仅仅是计算机化的部分，也包括与它交互的人）来预测——一种被称为*系统思考*的方法。我们可以尝试理解数据分析系统如何响应不同的行为、结构或特性。系统是加强扩大了现有的人与人之间的差异（比如说，使富人更富穷人更穷），还是试图与不公正现象作斗争？即使是出于最好的意图，我们也必须警惕意外的后果。

### 隐私与追踪

除了预测分析的问题——也就是，使用数据对人进行自动化决策——数据收集本身也存在伦理问题。收集数据的组织与被收集数据的人之间有什么关系？

当系统只储存用户显式输入的数据，是因为他们希望系统以某种方式储存和处理数据，系统正在为用户执行服务：用户是客户。但是，当用户的活动作为他们正在做的其他事情的副作用而被跟踪记录时，这种关系就不那么清晰了。服务不再只是做用户让它做的事情，而是根据自己的利益做的，而这可能与用户的利益相冲突。

追踪行为性质的数据对于许多在线服务面向用户的功能已经变得越来越重要：追踪点击了哪些搜索结果有助于提高搜索结果的排名；推荐“喜欢X的人也喜欢Y”可以帮助用户发现有趣有用的东西；A/B测试与用户流分析可以帮助用户界面如何改进。这些特性需要对用户行为进行一定量的跟踪，用户也从中受益。

然而跟踪取决于一家公司的商业模式，它往往不会止步于此。如果服务是通过广告资助的，那么广告商就是实际的客户，用户的利益排在第二位。跟踪数据变得更加详细，分析变得更加深入，数据被保存了很长一段时间，以便为营销目的建立每个人的详细概况。

这时，企业与被搜集数据的用户间的关系就看起来完全不同了。用户得到了免费的服务，并被劝说尽可能多地参与其中。对用户的跟踪并不主要服务于该个人，而是服务于为该服务提供资金的广告商的需求。我认为这种关系可以用一个更邪恶的词来描述：*监视*。

#### 监视

作为一个思考实验，试着用*监视*来代替*数据*这个词，然后观察常见的短语是否仍然听上去那么好。这个怎么样：“在我们的监视驱动的组织中，我们收集实时监视流，然后把它们存储在我们的监视仓库中。我们的监视科学家利用先进的分析与监视处理方法来获得新的洞察力。“

这个思想实验对于这本书来说是特别容易引起争论的，*设计监视密集型的应用程序*，但我认为需要强有力的词来强调这一点。在我们尝试使软件“吞噬世界”时，我们构建了有史以来世界上最大规模的监控基础设施。在奔向物联网时，我们正迅速走进这样一个世界，在这个世界里，每个有人居住的空间都至少有一个与互联网连接的麦克风，它们以智能手机、智能电视、语音控制辅助设备、婴儿监视器的形式出现，甚至还有使用基于云的语音识别的儿童玩具。许多这样的设备中都有着糟糕的安全记录。

即使是最极权和最专制的政权，也只能梦想在每个房间里放一个麦克风，强迫每个人携带能够可以不断追踪他们的位置与行动的装置。然而，我们显然是自愿、甚至是热情地投入到这个完全监视的世界中来的。不同的是，数据是由公司而不是由政府机构收集的。

并不是所有的数据收集都一定符合监视的条件，但是这样的检查可以帮助我们理解我们与数据收集者之间的关系。为何我们看上去乐于接受公司的监视呢？也许你觉得自己没什么可隐瞒的——换句话说，你完全符合现有的权力结构，你不是一个边缘化的少数群体，你不需要担心迫害。然而不是每个人都这么幸运。或者可能是因为这样做的目的似乎是温和的——它不是公开的强制和一致，而只是为了更好的推荐以及更个性化的营销。然而，结合上一节对预测分析的讨论，这种区别似乎不那么清晰。

我们已经看到了与车内跟踪设备相关的汽车保费，以及依赖于佩戴健身跟踪设备的人的健康保险覆盖范围。当监视被用来确定对生活的重要方面有影响的事情时，比如保险范围或就业，它就开始显得不那么友好了。此外，数据分析可以暴露令人惊讶的侵入性：比如，智能手表或是健身跟踪器中的运动传感器可以用来计算出您正在键入的内容（比如说密码），它具有相当高的准确性。而分析的算法只会变得越来越好。

#### 同意与选择的自由

我们也许会说，用户是自愿选择使用跟踪其活动的服务的，他们已经同意了服务条款和隐私政策，所以他们同意收集数据。我们甚至可以说，用户正在接受有价值的服务，以换取他们提供的数据，并且为了提供服务，跟踪是必要的。毫无疑问，社交网络、搜索引擎以及其他各种免费在线服务对用户来说都是有价值的——但这种论点也存在问题。

用户几乎不知道他们提供的哪些数据进入了我们的数据库，或者是这些数据是如何保存和处理的——而大多数隐私政策更多的是为了模糊而不是说明。不了解他们的数据发生了什么，用户就无法给予任何有意义的同意。通常，来自一个用户的数据也包含一些关于其他人的事情，这些人不是服务的用户，他们没有同意任何条款。我们在书的这一部分中讨论的衍生数据集——在这里，来自整个用户基础的数据可能已经与行为跟踪以及外部数据源相结合——恰恰是用户无法理解的数据。

此外，数据是通过单向过程从用户那里提取的，而不是真正的互惠关系，也不是公平的价值交换。没有对话，没有可以让用户协商他们提供多少数据以及他们得到什么回报服务的选项：服务和用户之间的关系是非常不对称和片面的。这些条款是由服务设置的，而不是由用户设置的。

对于不同意监视的用户来说，唯一的选择就是不使用服务。但这一选择也不是免费的：如果一项服务如此受欢迎，以至于“大多数人认为它对基本社会活动至关重要”，那么期望人们放弃这项服务是不合理的——使用它*实际上*是强制性的。例如在大多数西方社会中，携带智能手机、使用Facebook进行社交以及使用谷歌查找信息已成为常态。特别是当一项服务有了网络效应时，人们选择不使用它就会付出社会代价。

由于追踪用户而拒绝使用某项服务，这只是一小部分有足够的时间和知识来了解其隐私政策的人的选择，谁能承担得起可能错失出现在服务中的社交参与或是专业机会。对于处于弱势地位的人来说，根本没有选择自由：监视变得不可避免。

#### 隐私与数据的使用

有些时候人们说“隐私已经死了”，理由是有些用户愿意在社交媒体上发布他们生活中的各种事情，有时是普通的，有时是非常私人的。然而，这个说法是错误的，它基于对*隐私*这个词的错误理解。

拥有隐私并不意味着把一切当作秘密，它意味着有自由选择向谁透露哪些事情，向谁公开，以及保守什么秘密。隐私权是一项决策权：它使每个人都能决定在每一种情况下保密和透明之间的位置。这是一个人自由自主的一个重要方面。

当通过监视基础设施从人们那里获取了数据，隐私权并不一定会被削弱，而是转移到数据收集者那里。获得数据的公司事实上在说“相信我们对你的数据做了正确的事情”，这意味着决定披露什么、保密什么的权利从个人转移到了企业。

相应地，这些公司选择将这一监视结果的大部分保密，因为披露会被认为是令人毛骨悚然的，并会损害他们的商业模式（这种模式比其他公司更多地依赖于对人的了解）。关于用户的私人信息只是间接地被披露，例如以工具的形式针对特定人群（比如患有某种疾病的人（进行广告宣传。

即使某些特定用户无法从某一特定广告所针对的人的类型中重新确认身份，他们也失去了披露某些个人信息的代理机构，例如他们是否患有某种疾病。并不是用户根据自己的个人喜好来决定向谁透露了什么——而是公司以实现利润最大化为目标行使隐私权。

许多公司的目标是不被认为是令人毛骨悚然的——避免了对他们的数据收集实际上是有多么入侵性的问题，而是专注于管理用户感知。甚至这些感知通常管理得很差：例如，某些事情可能事实上是正确的，但是如果触发了痛苦的会议，用户可能并不想提到它。对于任何类型的数据，我们都应该期望它在某种程度上是错的，不需要的或是不合适的，我们需要建立处理这些错误的机制。某些事情到底是“不受欢迎的”还是“不恰当的”当然取决于人类的判断; 算法无法理解这些概念，除非我们编程，明确地让它们尊重人类需求。作为这些系统的工程师，我们必须谦虚，接受失败并为它们早做计划。

在线服务允许用户控制其他用户可以看到它的数据的哪些方面的隐私设置把一些控制权交还给了用户，这是一个开始。但是，无论设置如何，服务本身仍然可以自由地访问数据，并且可以以隐私策略允许的任何方式自由使用它。即使服务承诺不向第三方出售数据，它通常也会授予自己在内部处理和分析数据时不受任何限制的权利，通常这些比显示给用户的要多很多。

这种从个人到企业的大规模隐私权转移在历史上是从没有见过的。监控一直存在，但在过去代价极高且手动，无法扩展无法自动化。信任关系一直存在，例如患者与医生之间，被告与律师之间——但在这些情况下，数据的使用受到道德，法律与监管限制的严格控制。互联网服务使得在没有有意义的同意的情况下收集大量敏感信息变得更加容易，并且在没有用户了解其私人数据发生了什么的情况下大规模地使用了它们。

#### 数据作为资产与力量

由于行为数据是用户与服务交互的副产品，因此有时被称为“数据耗尽”——仿佛数据是毫无价值的废物。从这个角度来看，行为分析和预测分析可以被看作是一种回收的形式，从原本会被丢弃的数据中提取有价值的东西。

更正确的做法是反过来看待它：从经济角度来看，如果有针对性的广告是为一项服务买单的，那么有关人的行为数据就是该服务的核心资产。在这种情况下，与用户交互的应用仅仅是吸引用户将越来越多的个人信息输入到监视基础结构的一种手段。令人愉快的人类创造力和社会关系经常在在线服务中得到体现，而数据抽取设备却在冷嘲热讽般地利用它们。

对于个人数据是一项宝贵资产的判断得到了存在数据经纪人这个事实的支持，这是一个秘密经营的行业，购买、汇总、分析、推断和转售侵入性的个人数据，主要是为了营销目的。初创企业的价值取决于他们的用户数量和“眼球”——也就是他们的监视能力。

因为数据很有价值，很多人都想要它。当然，企业也想要它——这就是为什么他们一开始就收集它的原因。但是政府也想获得它：通过秘密交易、胁迫、法律强制，或者干脆窃取它。当一家公司破产时，它收集的个人资料是出售的资产之一。此外，数据很难保证安全，因此违约行为经常令人不安地发生。

这些观察使批评者认为，数据不仅是一种资产，而且是一种“有毒资产”，或者至少是“危险物质”。即使我们认为我们有能力防止滥用数据，但无论何时我们收集数据，我们都需要平衡其利益与落入坏人之手的风险：计算机系统可能被犯罪分子或敌对的外国情报机构破坏，数据可能被内部人员泄露，公司可能落入不符合我们价值观、肆无忌惮的管理层手中，或是国家被一个对强迫我们交出数据毫不畏惧的政权接管。

在收集数据时，我们不仅需要考虑当今的政治环境，还需要考虑所有可能的未来政府。不能保证今后每个选出来的政府都会尊重人权和公民自由，因此，“it is poor civic hygiene to install technologies that could someday facilitate a police state”。

俗话说：“知识就是力量。”此外，“审查他人而又避免自我审视是权力最重要的形式之一”。这就是极权主义政府想要监视的原因：它赋予了他们控制人们的权力。尽管今天的科技公司并不公开寻求政治权力，但他们积累的数据和知识却给了他们在我们生活中的大量权力，其中许多是不受公众监督的，远在公共视野之外。

#### 记住工业革命

数据是信息时代的决定性特征。互联网、数据存储、处理和软件驱动的自动化正在对全球经济和人类社会产生重大影响。由于我们的日常生活和社会组织在过去十年中已经发生了变化，而且在今后几十年里可能会继续发生根本性的变化，人们想到了与工业革命的比较。

工业革命是由于技术和农业的重大进步而产生的，从长远来看，它带来了持续的经济增长和生活水平的显著提高。然而，它也带来了重大问题：空气污染（由于烟雾和化学处理）和水污染（来自工业和人类废物）是可怕的。工厂老板过着富丽堂皇的生活，而城市工人往往住在很差的房子里，在艰苦的条件下长时间工作。童工现象很普遍，包括在矿山从事危险和低报酬的工作.

这花了很长时间才能建立起来保障措施，如环境保护条例、工作场所安全规程、取缔童工，以及食品卫生检查等。毫无疑问，当工厂不能再把垃圾倒入河流、出售受污染的食品或剥削工人时，做生意的成本就会增加。但作为一个整体，整个社会受益颇丰，我们中很少有人愿意回到那些监管之前的时代。

正如工业革命有需要管理的阴暗面一样，我们向信息时代的过渡也存在着我们需要面对和解决的重大问题。我认为数据的收集和使用就是这些问题之一。用布鲁斯·施耐尔的话说：

    数据是信息时代的污染问题，保护隐私是环境的挑战。几乎所有的计算机都会产生信息。它还在附近，在溃烂。我们如何处理它——我们如何控制它和如何处理掉它——对于我们信息经济的健康至关重要。就像我们今天回顾工业时代的最初几十年，并怀疑我们的祖先在匆忙建立一个工业世界时如何忽视污染一样，我们的孙辈们将在信息时代的最初几十年里回顾我们，并评价我们如何应对数据收集和滥用的挑战。我们应该努力让他们骄傲。

#### 立法与自我规范

数据保护法规可能有助于维护个人权利。例如，1995年“欧洲数据保护指令”规定，个人数据必须“为特定、明确和合法的目的收集，不得以不符合这些目的方式进一步处理”，此外，数据必须“与收集目的有关，而且不过分”。

然而，值得怀疑的是，这项立法在今天的互联网环境下是否有效。这些规则直接违背了大数据的理念，即最大限度地收集数据，将其与其他数据集相结合，进行实验和探索，以便产生新的见解。探索意味着将数据用于不可预见的目的，这与使用者表示同意的“具体的明确的”目的相（如果我们可以有意义地表示同意）。更新后的条例目前正在制定中。

收集了大量人们数据的公司反对监管，认为它是创新的负担与障碍。在某种程度上，这种反对是有道理的。例如，在分享医疗数据时，有明显的隐私风险，但也有潜在的机会：如果数据分析能够帮助我们获得更好的诊断结果，或是找到更好的治疗方法，那么可以避免多少人死亡？过度监管可能会阻止这种突破。这种潜在机会和风险是很难平衡的。

从根本上说，我认为我们需要科技行业在个人数据方面进行文化转变。我们应该停止将用户视为要优化的指标，并记住他们是值得尊重、尊严和灵活的人。我们应该自我规范我们的数据收集和处理实践，以构建、维护依赖我们软件的人们的信任。我们应该承担起自己的责任，教育终端用户如何使用他们的数据，而不是让他们处于黑暗之中。

我们应该允许每个人维护他们的隐私——也就是，他们对自己数据的控制——而不是通过监视从他们那里窃取控制权。我们个人控制数据的权利就像国家公园的自然环境：如果我们不明确地保护和爱护它，它就会被摧毁。这将是每一个人的悲剧，我们都会因此而变得更糟。无处不在的监视并不是不可避免的——我们仍然能够阻止它。

我们如何才能做到这一点是一个悬而未决的问题。首先，我们不应该永远保留数据，而是在不再需要它时立即清除。清除数据违背了不变性的概念（见“不变性的限制”一节），但这个问题是可以解决的。我看到的一种很有希望的方法是通过加密协议来执行访问控制，而不仅仅是通过策略来实现访问控制。总的来说，文化以及态度的改变是必要的。

## 小结

在这一章中，我们讨论了设计数据系统的新方法，并且把我个人的观点和对未来的展望包含在了里边。我们首先观察到，没有一款工具能够独自有效地为所有的用例服务，因此应用程序需要组合几个不同的软件来实现它们的目标。我们讨论了如何通过批处理和事件流让数据变更在不同系统之间流动来解决这个*数据集成*问题。

在这种方法中，特定一些系统被指定为保存记录的系统，而其他数据是通过转换从它们派生出来的。通过这种方式，我们可以维护索引、物化视图、机器学习模型、统计摘要等等。通过使这些派生和转换过程是异步松散耦合的，一个领域的问题不会扩展到系统的不相关的部分，从而提高了整个系统的健壮性和容错性。

把数据流表示为从一个数据集到另一个数据集的转换也有助于应用的发展：如果你希望更改其中一步处理步骤，比如更改索引或是缓存的结构，你可以在整个输入数据集上重新运行新的转换代码，从而重新派生输出。类似地如果某些地方出了问题，你可以修复代码然后重新处理数据来恢复。

这些过程与数据库内部已经实现的功能非常相似，因此我们把数据流应用程序重新定义为*解构*数据库的组件，并通过组合这些松散耦合的组件构建应用。

可以通过观察底层数据中的变更来更新衍生状态。此外，下游消费者还可以进一步观察到派生状态本身。我们甚至可以将这种数据流一路带到显示数据的终端用户设备，从而构建可以反映数据变化的动态更新用户界面，并且可以在脱机状态下继续工作。

接下来，我们讨论了如何确保在出现故障时所有这些处理还是正确的。我们发现通过使用端到端操作标识符使操作具有幂等性以及异步检查约束条件，可以用异步事件处理来实现强完整性保证。客户端可以等待检查通过，也可以不等待就继续进行，但这有可能在稍后为违反约束的行为道歉。与传统的使用分布式事务的方法相比，这种方法具有更高的伸缩性和更强的健壮性，也符合许多实际工作的业务流程。

通过围绕数据流构建应用并且异步地检查约束条件，我们可以避免大多数协调工作，构建出既保持了完整性且仍然运行良好的系统，即使是在地理分布的场景以及存在故障的情况下也是如此。然后，我们讨论了如何使用审计来验证数据的完整性以及检测数据破坏。

最后，我们退了一步，研究了构建数据密集型应用的一些道德方面问题。我们看到，尽管数据可以被用来做好事，但它也可能造成重大损害：作出合理但是严重影响人们的生活且难以上诉的决策，导致歧视和剥削，使监视变得常态化，还有揭露亲密信息。我们还面临着数据泄露的风险，我们也会发现即使善意地使用数据也会产生意想不到的后果。

由于软件和数据对世界产生了如此大的影响，我们工程师必须记住，我们有责任为我们想要生活的那种世界而努力：一个以人道和尊重对待人们的世界。我希望我们能够为实现这一目标而共同努力。